...
DomainObjectsCollection::DomainObjectsCollection(int aircraft_number) {
    ...
    for (int i = 0; i < aircraft_number; i++) {
        ...
        objects.push_back(std::make_shared<Aircraft>(Aircraft(track, AIRCRAFT_VELOCITY, i)));
    }
    auto air_crafts = aircrafts();
	ConflictManager_ptr conflict_manager = std::make_shared<ConflictManager>(ConflictManager(air_crafts));
    objects.push_back(std::dynamic_pointer_cast<DomainObject>(conflict_manager));
	objects.push_back(std::dynamic_pointer_cast<DomainObject>(std::make_shared<ConflictSolver>(ConflictSolver(conflict_manager))));
}
